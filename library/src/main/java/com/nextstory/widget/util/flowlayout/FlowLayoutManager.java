...
import com.nextstory.widget.util.flowlayout.cache.CacheHelper;
import com.nextstory.widget.util.flowlayout.cache.Line;
...
public class FlowLayoutManager extends RecyclerView.LayoutManager {
    ...
    private void onPreLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int currentItemPosition = Math.max(firstItemAdapterPosition, 0);
        ...
        // this option use old options alignment & new options line limit to calc items for
        // animation.
        ...
        // track before removed and after removed layout in same time, to make sure only add items
        // at bottom that visible after item removed.
        while (currentItemPosition < state.getItemCount()) {
            ...
            if (!childRemoved) {
                realNewline = calcChildLayoutRect(
                        child, realX, realY, realHeight, afterContext, realRect);
                if (realNewline) {
    ...
    private void onRealLayoutChildren(RecyclerView.Recycler recycler) {
        ...
        for (int i = firstChildAdapterPosition; i < itemCount; i++) {
            ...
            if (!childVisible(false, rect)) {
                ...
                addView(child);
                appenders.add(new LayoutManagerAppender(
                        child, this, rect, flowLayoutOptions.alignment));
    ...
    public boolean canScrollVertically(final int direction) {
        if (direction < 0) {
            ...
            return !(getChildAdapterPosition(firstChild) == 0
                    && getDecoratedTop(topChild) >= topVisibleEdge());
        } else {
            View lastChild = getChildAt(getChildCount() - 1);
            View bottomChild = getChildAt(getMaxHeightLayoutPositionInLine(getChildCount() - 1));
            return !(getChildAdapterPosition(lastChild)
    ...
    public void onItemsChanged(@NonNull RecyclerView recyclerView) {
        ...
        cacheHelper = new CacheHelper(
                flowLayoutOptions.itemsPerLine, layoutHelper.visibleAreaWidth());
    ...
    private int contentMoveUp(int dy, RecyclerView.Recycler recycler) {
        ...
        int offscreenBottom = getDecoratedBottom(maxHeightItem)
                - (getClipToPadding() ? bottomVisibleEdge() : getHeight());
        while (offscreenBottom < dy
                && getChildAdapterPosition(getChildCount() - 1) < getItemCount() - 1) {
    ...
    private int contentMoveDown(int dy, RecyclerView.Recycler recycler) {
        ...
        // scrolling
        int offScreenTop = (getClipToPadding() ? topVisibleEdge() : 0)
     ...
     * Here is an example to explain why: Say total width is 10, current line is [5, 2, 1],
     * 3 numbers before current line is 7, 2, 6. If you just look back, you could say previous line
     * [2, 6], but there is another possibility: [7, 2], [6]. You don't know which one is correct
     * without knowing how the line before previous one, same thing could happen to that line too,
     * so you have to sort everything from beginning or cache the result.
     */
    private void addNewLineAtTop(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < lineChildren.size(); i++) {
            ...
            appenders.add(new LayoutManagerAppender(
                    childView, this, rect, flowLayoutOptions.alignment));
            x = advanceInSameLine(x, rect, layoutContext);
        }
    ...
    private void addNewLineAtBottom(RecyclerView.Recycler recycler) {
        ...
        while (childAdapterPosition < getItemCount()) {
            ...
            if (newline && !firstItem) {
                ...
                addView(newChild);
                appenders.add(new LayoutManagerAppender(
                        newChild, this, rect, flowLayoutOptions.alignment));
                x = advanceInSameLine(x, rect, layoutContext);
    ...
    public void onAttachedToWindow(final RecyclerView view) {
        ...
        cacheHelper = new CacheHelper(
                flowLayoutOptions.itemsPerLine,
    ...
    private boolean childVisible(boolean preLayout, int left, int top, int right, int bottom) {
        ...
        return Rect.intersects(
                new Rect(
                        leftVisibleEdge(),
                        clip ? topVisibleEdge() : 0,
                        rightVisibleEdge(),
                        clip ? bottomVisibleEdge() : getHeight()),
    ...
    private int getMaxHeightLayoutPositionInLine(int layoutPosition) {
        try {
            ...
            int maxIndexBefore = layoutPosition,
                    maxIndexAfter = layoutPosition,
                    maxHeightBefore = getDecoratedMeasuredHeight(child),
    ...
    private int getOffsetOfItemToFirstChild(int adapterPosition, RecyclerView.Recycler recycler) {
        ...
        if (adapterPosition > firstChildPosition) {
            ...
            if (lastChildAdapterPosition >= adapterPosition) {
                int targetChildIndex = getChildCount()
                        - 1
    ...
    private boolean calcChildLayoutRect(View child,
                                        int x, int y,
                                        int lineHeight,
        ...
        switch (layoutContext.layoutOptions.alignment) {
            case RIGHT: {
                boolean isStartNewLine = LayoutHelper.shouldStartNewline(
                        x, childWidth, leftVisibleEdge(), rightVisibleEdge(), layoutContext);
                if (isStartNewLine) {
                    newLine = true;
    ...
    private boolean isStartOfLine(int index, LayoutContext layoutContext) {
        if (index == 0) {
            ...
            switch (layoutContext.layoutOptions.alignment) {
                ...
                case CENTER:
                default:
                    return getDecoratedTop(
                            getChildAt(index)) > getDecoratedTop(getChildAt(index - 1));
            }
        }
    }
    ...
    private boolean isEndOfLine(int index, LayoutContext layoutContext) {
        return LayoutHelper.hasItemsPerLineLimit(layoutContext.layoutOptions)
                && layoutContext.currentLineItemCount == layoutContext.layoutOptions.itemsPerLine
                || getChildCount() == 0
                || index == getChildCount() - 1
                || isStartOfLine(index + 1, layoutContext);
    }
